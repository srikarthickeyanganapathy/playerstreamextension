...
export class FastStreamPlayer {
    ...
    async _loadHLS(url) {
        ...
        // Prefer HLS.js over native for better quality control and features
        if (typeof Hls !== 'undefined' && Hls.isSupported()) {
            this._log('HLS.js is available, using it for playback');
            return this._loadHLSWithLibrary(url);
        }

        // Fallback: Check for native HLS support (Safari, iOS, some mobile browsers)
        const canPlayNative = this.video.canPlayType('application/vnd.apple.mpegurl');
        if (canPlayNative === 'probably' || canPlayNative === 'maybe') {
            this._log('Using native HLS support (Safari/iOS)');
            return this._loadNative(url);
        }

        // No HLS support available
        throw new Error('HLS playback not supported: HLS.js failed to load and browser has no native HLS support');
    }

    /**
     * Load HLS stream using HLS.js library
     * @param {string} url - HLS manifest URL  
     */
    async _loadHLSWithLibrary(url) {
...
class PlayerPageController {
    ...
    async init() {
        ...
        this.elements = {
            ...
            copyUrlControlBtn: document.getElementById('copyUrlControlBtn'),
        ...
        this.currentUrl = url; // Store the URL for copy functionality
        if (url) {
    ...
    _bindUIEvents() {
        ...
        if (this.elements.copyUrlControlBtn) {
            this.elements.copyUrlControlBtn.addEventListener('click', () => {
                this._copyUrlToClipboard();
            });
        }
    ...
    /**
     * Copy the current video URL to clipboard
     */
    async _copyUrlToClipboard() {
        if (!this.currentUrl) {
            console.warn('[FastStream] No URL to copy');
            return;
        }

        try {
            await navigator.clipboard.writeText(this.currentUrl);
            this._showCopyToast('URL copied to clipboard!');
            console.log('[FastStream] URL copied:', this.currentUrl);
        } catch (err) {
            ...
            try {
                ...
                this._showCopyToast('URL copied to clipboard!');
                console.log('[FastStream] URL copied (fallback):', this.currentUrl);
            } catch (fallbackErr) {
                this._showCopyToast('Failed to copy URL');
                console.error('[FastStream] Failed to copy URL:', fallbackErr);
            }
        }
    }
    ...
    _showCopyToast(message) {
        if (this.elements.copyToast) {
            this.elements.copyToast.textContent = message;
            this.elements.copyToast.classList.add('visible');

            // Hide after 2 seconds
            setTimeout(() => {
                this.elements.copyToast.classList.remove('visible');
            }, 2000);
        }
    }
    ...
    async _retryLoad() {
        if (!this.currentUrl) {
            console.warn('[FastStream] No URL to retry');
            return;
        }
        ...
        try {
            // Destroy and reinitialize the player
            if (this.player) {
                this.player.destroy();
            }
